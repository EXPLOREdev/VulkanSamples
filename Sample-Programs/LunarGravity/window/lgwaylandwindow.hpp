/*
 * LunarGravity - lgwaylandwindow.hpp
 *
...
class LgWaylandWindow : public LgWindow {
    public:

        // Create a protected constructor
        LgWaylandWindow(const char *win_name, const uint32_t width, const uint32_t height, bool fullscreen);

        // We don't want any copy constructors
        LgWaylandWindow(const LgWaylandWindow &window) = delete;
        LgWaylandWindow &operator=(const LgWaylandWindow &window) = delete;

        // Make the destructor public
        virtual ~LgWaylandWindow();
